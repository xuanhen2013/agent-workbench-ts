...
import {
  ...
  importInterviewJd,
  searchMarketJds,
  submitInterviewQuizAnswersStream,
} from './api'
import { InterviewQuizGraph } from './InterviewQuizGraph'
...
export function InterviewQuizDemo() {
  ...
  const [activities, setActivities] = useState<QuizActivityEvent[]>([])
  const [error, setError] = useState<string>()

  const appendActivity = (activity: QuizActivityEvent) => {
    setActivities(current => [...current.slice(-199), activity])
  }
  ...
  async function submitAnswers() {
    ...
    try {
      ...
      const nextView = await submitInterviewQuizAnswersStream(
        view.threadId,
        round.reviewId,
      ...
      <div className="quiz-workspace-grid">
        <div className="quiz-left-column">
          <header className="quiz-page-header">
            <div>
              <p className="quiz-page-kicker">Agent 面试训练</p>
              <h1>边答题，边看 Graph 运行</h1>
              <p>
                左侧完成配置和答题，右侧实时显示 Parent Graph、Planning Subgraph 与 Tool Loop。
              </p>
            </div>
            {view && (
              <dl className="quiz-session-meta">
                <div>
                  <dt>Thread</dt>
                  <dd title={view.threadId}>{view.threadId.slice(0, 8)}</dd>
                </div>
                <div>
                  <dt>Rounds</dt>
                  <dd>{`${view.results.length}/${view.config.maxRounds}`}</dd>
                </div>
              </dl>
            )}
          </header>

          <section className="quiz-primary" aria-label="面试题答题区">
